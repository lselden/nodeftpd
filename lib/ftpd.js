...
var FtpServer = function (cfg) {
	...
	this.root = this.root || "./test"; // change to root for filesystem
...
var commands = {
	...
	*/
	...
	"MKD":  function () { this.reply(502); }, // Make directory.
	...
	"PASS": function (password) {
		...
		socket.emit('command:pass', socket.username, function (err) {
			if (err) {
	...
	"PASV": function () { // Enter passive mode
		...
		dataListener.on('connection', function (dataSocket) {
			...
			dataSocket.on('data', function (data) {
				...
				socket.logIf(4, "Data event: received " + (Buffer.isBuffer(data) ? "buffer" : "string"), socket);
			});
		...
		dataListener.listen(dataPort, function () {
			...
			socket.reply(227, ["PASV OK (", host.split(".").join(","), ",", parseInt(port / 256, 10), ",", (port % 256), ")"].join(''));
	...
	"LIST": function (target) {
		...
		socket.dataTransfer(function (dataSocket, callback) {
			...
			socket.fs.list(target || socket.fs.pwd(), function (err, result) {
				...
				// pipe stream, or just write straight to socket
	...
	"STOR": function (file) {
		...
		socket.dataTransfer(function (dataSocket, callback) {
			...
			socket.emit('command:stor:begin');
